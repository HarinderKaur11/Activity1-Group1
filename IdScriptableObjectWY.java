    ...
    implements IdFunctionCall
{
    ...
    private static final class PrototypeValues implements Serializable
    {
        ...
        final void initValue(int id, String name, Object value, int attributes)
        {
            ...
            if (id == constructorId) {
                ...
                constructorAttr = (short)attributes;
                return;
            }
        ...
        final Object[] getNames(boolean getAll, Object[] extraEntries)
        {
            ...
            if (count == 0) {
                ...
                if (count != names.length) {
                    Object[] tmps = new Object[count];
                    System.arraycopy(names, 0, tmps, 0, count);
                    names = tmps;
                }
                return names;
            } else {
                int extra = extraEntries.length;
                Object[] tmps = new Object[extra + count];
                System.arraycopy(extraEntries, 0, tmps, 0, extra);
                System.arraycopy(names, 0, tmps, extra, count);
                return tmps;
            }
        }
    ...
    public int getAttribute(String name)
    {
        ...
        if (prototypeValues != null) {
            int id = prototypeValues.findId(name);
            if (id != 0) {
                return prototypeValues.getAttribute(id);
            }
        }
    ...
    public void setAttributes(String name, int attributes)
    {
        ...
        if (info != 0) {
            int id = (info & 0xFFFF);
            int currentAttribute = (info >>> 16);
            if (attributes != currentAttribute) {
    ...
    {
        Object[] results = super.getIds(getNonEnumerable, getSymbols);

        if (prototypeValues != null) {
            results = prototypeValues.getNames(getNonEnumerable, results);
        }
        ...
        if (maxInstanceId != 0) {
            ...
            if (count != 0) {
                if (results.length == 0 && ids.length == count) {
                    results = ids;
                }
                else {
                    Object[] tmps = new Object[results.length + count];
                    System.arraycopy(result, 0, tmps, 0, results.length);
                    System.arraycopy(ids, 0, tmps, results.length, count);
                    results = tmps;
                }
            }
        }
        ...
        if (prototypeValues != null) {
            ...
            if (id != 0) {
              if (isAccessorDescriptor(desc)) {
                ...
                ScriptableObject current = getOwnPropertyDescriptor(cx, key);
                checkPropertyChange(name, current, desc);
                int attr = prototypeValues.getAttribute(id);
                Object value = getProperty(desc, "value");
                if (value != NOT_FOUND && (attr & READONLY) == 0) {
    ...
    private ScriptableObject getBuiltInDescriptor(String name) {
      ...
      if (prototypeValues != null) {
        ...
        if (id != 0) {
          value = prototypeValues.get(id);
          attr = prototypeValues.getAttribute(id);
          return buildDataDescriptor(scope, value, attr);
        }
      }
      return null;
...
public class IdScriptableObjectWY {
