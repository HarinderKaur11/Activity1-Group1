...
public class DirectoryScanner
    ...
    public synchronized void addExcludes(final String[] excludes) {
        if (excludes != null && excludes.length > 0) {
            if (this.excludes != null && this.excludes.length > 0) {
                final String[] tmps = new String[excludes.length
                                          + this.excludes.length];
                System.arraycopy(this.excludes, 0, tmps, 0,
                                 this.excludes.length);
                for (int i = 0; i < excludes.length; i++) {
                    tmps[this.excludes.length + i] =
    ...
    public void scan() throws IllegalStateException {
        ...
        try {
            synchronized (this) {
                ...
                final boolean nullInclude = (include == null);
                include = nullInclude
                    ? new String[] {SelectorUtils.DEEP_TREE_MATCH} : include;
                final boolean nullExclude = (exclude == null);
                exclude = nullExclude ? new String[0] : exclude;
                ...
                clearCaches();
                include = nullInclude ? null : include;
                exclude = nullExclude ? null : exclude;
            }
    ...
    public synchronized void addDefaultExcludes() {
        ...
        String[] newExcludes;
        final String[] defaultExcludesTemps = getDefaultExcludes();
        newExcludes = new String[excludesLength + defaultExcludesTemps.length];
        if (excludesLength > 0) {
            System.arraycopy(excludes, 0, newExcludes, 0, excludesLength);
        }
        for (int i = 0; i < defaultExcludesTemps.length; i++) {
            newExcludes[i + excludesLength] =
                defaultExcludesTemps[i].replace('/', File.separatorChar)
                .replace('\\', File.separatorChar);
        }
    ...
    /* package */ synchronized void ensureNonPatternSetsReady() {
        if (!areNonPatternSetsReady) {
            includePatterns = fillNonPatternSet(includeNonPatterns, include);
            excludePatterns = fillNonPatternSet(excludeNonPatterns, exclude);
